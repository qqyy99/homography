...
int main( int argc, char** argv)
{
    ...
    Mat HMat = findHomography(pts_src, data.points);
    
    // 用H对原图做变换

    warpPerspective(im_src, im_temp, HMat, im_temp.size());
    ...
    // 把目标图中对应区域像素值设置为0
    fillConvexPoly(im_dst, pts_dst, 4, cv::Scalar(0, 0, 0), CV_AA);
